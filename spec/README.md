...
owns compilation, accessibility, internal links, content references and API
generation.

`course-api.test.ts` asserts the generated `/api` JSON against
`fixtures/golden-api/`, captured from the original Astro build before this repo
was ported to Next.js. The API is the contract with the programs and courses
page, so it survived the port unchanged and this test is what proves it.

`build-gates.test.ts` covers the gate scripts themselves.
...
file alongside the supplied ones (any `spec/*.test.ts` runs with `bun run
check`, on Bun's own test runner --- `import { describe, it, expect } from
"bun:test"`).
